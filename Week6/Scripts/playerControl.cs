...
{   
  ...
  // called when the cube hits the floor
    ...
    void Start()
    {
      ...
      GameManagers.OnPlayerDeath += PlayerDiesSequence ; 
    ...
    void FixedUpdate()
    {
      ...
      // dynamic rigidbody
        ...
        if (Mathf.Abs(moveHorizontal) > 0){
            ...
            if (marioBody.velocity.magnitude < constants.playerMaxSpeed)
        ...
        if (Input.GetKeyDown("space") && onGroundState){
          marioBody.AddForce(Vector2.up * constants.playerMaxJumpSpeed, ForceMode2D.Impulse);
          onGroundState = false;
          marioAnimator.SetBool("onGround", onGroundState) ; 
